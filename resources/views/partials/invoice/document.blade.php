...
<div class="print-area rounded-lg border border-gray-200 bg-white p-6 shadow-sm sm:p-8">
            ...
            <div class="mt-1 flex items-center justify-end gap-2">
                @if ($invoice->isPaid())
                    ...
                    @if ($invoice->payments->isNotEmpty())
                        <span class="text-xs text-gray-500">{{ __('invoice.paid') }} on {{ format_date($invoice->payments->first()->paid_at) }}</span>
                    @endif
                @else
    ...
    <div class="mt-6 grid grid-cols-1 gap-6 border-t border-gray-100 pt-6 sm:grid-cols-2">
        <div>
            ...
            <p class="mt-2 text-sm font-medium text-gray-900">{{ config('app.owner.name') }}</p>
        ...
        <div>
            ...
            <p class="mt-2 text-sm font-medium text-gray-900">{{ $invoice->project->client_company ?: $invoice->project->client_name }}</p>
